...
contract ProtocolTokenLenderFirstResortNonProtToken is ReentrancyGuard {
    // --- Auth ---
    ...
    */
    function join(uint256 wad) public payRewards {
    ...
    */
    function exit(uint256 wad) public nonReentrant payRewards {
    ...
    // rewards already paid to the user
    mapping(address => uint256) public rewardDebt;
    uint256 public accTokensPerShare;
    uint256 public lastPoolUpdate = now;


    // should be called from deposits and withdrawals (whenever descendant balance changes)
    modifier payRewards() {
        // updates pool
        updatePool();

        if (descendant.balanceOf(msg.sender) > 0 && rewardDripper.rewardToken().balanceOf(address(this)) > 0) {

            // pays reward
            uint256 pending = subtract(multiply(descendant.balanceOf(msg.sender), accTokensPerShare), rewardDebt[msg.sender]);
            rewardDripper.rewardToken().transferFrom(address(this), msg.sender, pending);
        }
        _;
        rewardDebt[msg.sender] = multiply(descendant.balanceOf(msg.sender), accTokensPerShare) / 1e12;
    }

    // Update reward variables of the pool to be up-to-date.
    function updatePool() public {
        if (block.number <= lastPoolUpdate) return;
        uint balance = rewardDripper.rewardToken().balanceOf(address(this));
        pullReward();
        uint increaseInBalance = rewardDripper.rewardToken().balanceOf(address(this)) - balance;

        uint256 lpSupply = descendant.totalSupply();
        if (lpSupply == 0) {
            lastPoolUpdate = block.number;
            return;
        }

        // updates distribution info
        accTokensPerShare = addition(accTokensPerShare, increaseInBalance) / lpSupply;
        lastPoolUpdate = block.number;
    }
}
