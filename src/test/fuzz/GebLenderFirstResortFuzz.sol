...
contract Fuzz {
    ...
    function doJoin(uint wad) public createCaller {
        ...
        uint previousPoolBalance   = ancestor.balanceOf(address(stakingPool));
        uint previousCallerBalance = descendant.balanceOf(address(callers[msg.sender]));
        callers[msg.sender].doJoin(wad);
        assert(ancestor.balanceOf(address(stakingPool)) == previousPoolBalance + wad);
        assert(descendant.balanceOf(address(callers[msg.sender])) == previousCallerBalance + price);
    }
    ...
    // invariants
    function echidna_descendant_supply() public returns (bool) {
        return totalCallerBalance(descendant) == descendant.totalSupply();
    }
